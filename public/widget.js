(function () {
  ...
  style.textContent = `
    ...
    .cai-msg.user { background: #16a34a; color: #fff; align-self: flex-end; border-bottom-right-radius: 4px; }
    #cai-footer {
    ...
    #cai-input:focus { border-color: #16a34a; }
    #cai-send {
  ...
  async function loadNegocio() {
    ...
    if (data.length) {
      ...
      addMessage('bot', `¡Hola! Soy el asistente de ${negocio.nombre}. ¿En qué puedo ayudarte?`);
    }
  }
  ...
  function buildPrompt(n) {
    return `Eres el asistente virtual de "${n.nombre}".
Responde siempre en español, de forma amable y concisa (máximo 3 líneas).
Solo responde sobre el negocio.
${n.descripcion ? `DESCRIPCIÓN: ${n.descripcion}` : ''}
${n.menu ? `MENÚ / SERVICIOS:\n${n.menu}` : ''}
